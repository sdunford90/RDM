...
function extractParcel(feature) {
...
router.post('/', async (req, res) => {
  try {
    ...
    let url;
    if (lat && lng) {
      url = `https://app.regrid.com/api/v2/parcels/point?lat=${lat}&lon=${lng}&radius=50&token=${token}&returnGeo=true`;
    } else if (address) {
      url = `https://app.regrid.com/api/v2/parcels/address?query=${encodeURIComponent(address)}&token=${token}&returnGeo=true&limit=1`;
    } else {
    ...
    // Regrid v2 API nests results under data.parcels.features
    ...
    res.json(extractParcel(features[0]));
  } catch (err) {
